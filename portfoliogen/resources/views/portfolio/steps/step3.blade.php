...
@section('title','Create Portfolio - Step 3')
...
<div class="pg-step-wrapper">
  <div class="pg-step-box">
    ...
    <div class="pg-step-header">
      <div class="pg-step-title">Work Experience</div>
    ...
    <div class="pg-form-grid pg-exp-grid">
      <div class="pg-card">
        <form method="POST" action="{{ route('portfolio.step.save', [
          ...
          <label class="pg-label">Company Name *</label>
          <input id="expCompany" name="company_name" class="pg-input" value="{{ old('company_name') }}">
          @error('company_name')
            <div style="color:#ff6b6b; margin-bottom:12px;">{{ $message }}</div>
          @enderror

          <label class="pg-label">Role *</label>
          <input id="expRole" name="role" class="pg-input" value="{{ old('role') }}">
          @error('role')
            <div style="color:#ff6b6b; margin-bottom:12px;">{{ $message }}</div>
          @enderror

          <label class="pg-label">Description</label>
          <textarea id="expDesc" name="description" class="pg-input" rows="4" style="resize:vertical;">{{ old('description') }}</textarea>
          @error('description')
            <div style="color:#ff6b6b; margin-bottom:12px;">{{ $message }}</div>
          @enderror
      ...
      <div class="pg-card">
        ...
        <div class="pg-edu-head">Your Experiences</div>

        @if($portfolio->experiences->count() === 0)
          <div class="pg-muted">No experience added yet. You can continue next.</div>
        @else
          <div class="pg-edu-list">
            @foreach($portfolio->experiences as $e)
              @php $editId = "exp-edit-".$e->id; @endphp
              ...
              <div class="pg-edu-item" style="flex-direction:column; align-items:stretch;">
                ...
                <div style="display:flex; justify-content:space-between; gap:12px; align-items:flex-start; width:100%;">
                  <div class="pg-edu-content" style="width:100%;">
                    <div class="pg-edu-title">{{ $e->role }} — {{ $e->company_name }}</div>
                    ...
                    @if($e->description)
                      <div class="pg-muted" style="margin-top:8px; line-height:1.5;">
                        {{ $e->description }}
                      </div>
                    @endif
                  </div>
                  ...
                  <div style="display:flex; gap:10px; flex-shrink:0;">
                    ...
                    <form method="POST"
                          action="{{ route('portfolio.experience.delete', [
                            'username' => auth()->user()->username,
                            'portfolio' => $portfolio->id,
                            'experience' => $e->id
                          ]) }}"
                ...
                <div id="{{ $editId }}"
                  ...
                  <form method="POST" action="{{ route('portfolio.experience.update', [
                    'username' => auth()->user()->username,
                    'portfolio' => $portfolio->id,
                    'experience' => $e->id
                  ]) }}">
                    ...
                    <label class="pg-label">Company Name *</label>
                    <input name="company_name" class="pg-input" value="{{ $e->company_name }}">

                    <label class="pg-label">Role *</label>
                    <input name="role" class="pg-input" value="{{ $e->role }}">

                    <label class="pg-label">Description</label>
                    <textarea name="description" class="pg-input" rows="4" style="resize:vertical;">{{ $e->description }}</textarea>
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:15px;">
                      ...
                      <div>
                        ...
                        <div class="pg-muted" style="margin-top:6px;">Leave empty if currently working.</div>
                      </div>
                    </div>
...
<script>
  ...
  pgBindAiAutocomplete({ inputId:"expCompany", type:"company" });
  pgBindAiAutocomplete({ inputId:"expRole", type:"job_title" }); 

  document.getElementById("aiExpDescBtn")?.addEventListener("click", async () => {
    const btn = document.getElementById("aiExpDescBtn");
    const out = document.getElementById("aiExpDescOut");
...
Draft: ${desc?.value || ""}`;
    ...
    try{
      const suggestions = await pgAiSuggest("experience", text, {
        job_title: @json($portfolio->job_title)
      });
